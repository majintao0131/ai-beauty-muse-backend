...
class ChatService:
    ...
    async def chat(
        ...
        Returns:
            Dictionary containing AI reply and suggestions
        """
        # 調用 OpenAI 服務
    ...
    def _generate_suggestions(
        ...
        # 髮型相關
        if any(word in user_message.lower() or word in ai_reply for word in ["髮型", "頭髮", "劉海", "捲髮", "直髮"]):
            suggestions.extend([
                "這個髮型需要怎麼打理？",
                "有沒有其他類似的髮型推薦？",
                "這個髮型適合什麼臉型？",
        ...
        # 妝容相關
        if any(word in user_message.lower() or word in ai_reply for word in ["妝容", "化妝", "眼影", "唇色", "腮紅"]):
            suggestions.extend([
                "有沒有推薦的產品？",
                "這個妝容適合什麼場合？",
                "怎麼讓妝容更持久？",
        ...
        # 穿搭相關
        if any(word in user_message.lower() or word in ai_reply for word in ["穿搭", "衣服", "搭配", "風格", "款式"]):
            suggestions.extend([
                "有沒有具體的單品推薦？",
                "這個風格適合什麼場合？",
                "怎麼用配飾提升整體造型？",
        ...
        # 色彩相關
        if any(word in user_message.lower() or word in ai_reply for word in ["顏色", "色彩", "膚色", "冷調", "暖調"]):
            suggestions.extend([
                "我適合什麼顏色的髮色？",
                "有沒有百搭的顏色推薦？",
                "怎麼判斷自己的膚色調？",
